...
export default {
    ...
    blocks: [
        { h2: "Installation" },
        {
            p: "Drop Leafs into the `mods` folder of a Fabric 26.2 server running Java 25, together with Fabric API, Mapple, ScalableLux and FastNoise. Without one of these dependencies, the server refuses to start and says so clearly."
        },
        {
            p: "On first startup, Leafs writes `config/leafs.json` with the default values, and sets `sync-chunk-writes` to `false` in `server.properties`. The admin can set it back to `true`, and Leafs never touches it again after that."
        },
        { h2: "Threads and regions" },
        {
            table: {
            ...
            table: {
                ...
                rows: [
                    ...
                    [
                        ...
                        "`50`",
                        "Past this delay, a thread that waited for a chunk is logged, and so is an abnormally long mailbox task, with its class. From 0 to 60000."
                    ],
        ...
        {
            p: "The forced shutdown follows `max-tick-time` from `server.properties`, like vanilla. Past this delay on a region, Leafs writes a crash report with all threads and then kills the JVM. `-1` disables it, and in singleplayer there is no forced shutdown."
        },
        {
        ...
        { note: "A value changed with `/leafs config` takes effect on the next startup." },
        { h2: "In the code" },
        {
            p: "`LeafsConfig.register` reads the file, checks every range, and writes the defaults if it does not exist. `sectionShift()` is the logarithm of `section_size`, and `Regionizer` requires a shift between 1 and 8. `effectiveRegionThreads()` and `effectiveChunkThreads()` translate the `-1`. `ServerProperties` is the only point that touches `server.properties`, and it only does so on first startup."
        }
    ]
} satisfies DocPage;
