...
A region is not a thread. A region is a task. Regions wait in a single list, sorted by the time of their next tick. A free worker takes the first one and ticks it. A worker busy with a heavy region does not block anyone else, the others move on.
...
A region never waits on another thread. It only waits on the pool for a chunk, because the pool never waits on anything in return. A player, or an entity, is held by only one thread at a time. A region that finds a player held by another thread skips it and picks it up again on the next tick.

## Chunk workers

Chunk workers are independent from region workers. They generate, light, load and unload chunks, and prepare the bytes to be written to disk. Vanilla's disk thread now only reads and writes those bytes.

These workers run at the lowest system priority. When the machine runs short on resources, region ticks go first, because they have a 50 ms deadline to keep. Chunks take what is left.
...
3. For each dimension: the time, the weather, the border, the tickets and the players' view, unloads, special spawns like phantoms or the wandering trader, then a single request to the chunk workers for their pass over regionless chunks. Raids and the dragon fight tick on the region that owns their center.
...
6. The player list.
7. The autosave clock. It is then the regions and the chunk workers that save.
...
The number of workers comes from `region_threads` and `chunk_threads`. By default, all cores for regions and half for chunks.
