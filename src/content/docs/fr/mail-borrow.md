...
Une région empruntée ne tick pas pendant ce temps, comme si le thread serveur était son worker. C'est court : une connexion prend moins d'une milliseconde.

## Dans le code

### La boîte

`RegionInbox` garde deux `ArrayDeque`, `chunkWork` et `gameWork`. `drain()` sert le travail de chunk puis le travail de jeu, autant de tâches qu'il y en avait avant l'appel. Une tâche qui se reposte attend la passe suivante. `drain(deadline)` s'arrête à l'échéance.

`drainChunkWork()` est ce qu'une attente a le droit de faire tourner. Un thread qui attend un chunk peut ainsi atteindre la publication qui le bloque, sans jamais exécuter de travail de jeu au milieu d'une écriture. Une tâche posée sur un chunk que la région ne possède plus repart par `ChunkOwners.submit`.

### L'emprunt

`RegionBorrow` est ce qu'un thread tient pour un travail de jeu. Seule la tête, le thread serveur, attend et prend des régions. Tout autre thread ne fait que lire une région et n'attend jamais.
...
`DeferredWork` est une tâche de jeu qui part chez le propriétaire d'un chunk avec une raison, `DeferReason` : `RESPAWN`, `PORTAL`, `TELEPORT`, `PLAYER_TELEPORT` ou `BLOCK_WRITE`. Elle revalide sa condition à l'arrivée, par exemple que l'entité est encore vivante, et compte un abandon si elle échoue. `/leafs metrics` affiche ces compteurs.

`GlobalScheduler` est la file du thread serveur : tout `MinecraftServer.execute` venu d'un autre thread y atterrit, et le thread serveur la vide une passe à la fois.
