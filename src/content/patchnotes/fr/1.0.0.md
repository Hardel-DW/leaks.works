...
Première version. Le monde tick par régions, sur un pool de threads, à côté d'un thread serveur à coût fixe.

- Régions découpées en sections de `section_size` chunks, avec couronne, fusion et scission entre deux ticks.
- Un pool de threads de régions et un pool de threads de chunks indépendants, à priorité système minimale.
